...
function dueDateLabel(dateStr: string) {
  ...
  return { text: new Date(dateStr).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }), className: "text-gray-500 dark:text-slate-400" }
}
...
function Dropdown({ trigger, children, align = "left" }: {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className={`absolute z-50 mt-1 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg shadow-lg py-1 min-w-[140px] ${align === "right" ? "right-0" : "left-0"}`}
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#FAFAFA] dark:bg-slate-950 text-gray-900 dark:text-gray-100">
      ...
      <div className="px-8 py-5 border-b border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100 tracking-tight">Tasks</h1>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4">
          ...
          <Dropdown
            align="left"
            trigger={
              <button className="h-7 flex items-center gap-1 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-md px-2 text-[12px] font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-slate-900">
          ...
          >
            {[
              ...
              <button key={opt.value}
                ...
                className={`w-full text-left px-3 py-1.5 text-[12px] font-medium hover:bg-gray-50 dark:bg-slate-900 ${
                  sortBy === opt.value ? "text-[#6366F1] font-semibold" : "text-gray-700 dark:text-gray-300"
                }`}
              >
                {opt.label}
              </button>
      ...
      <div className="flex-1 overflow-auto">
        {isLoading ? (
          <div className="p-12 text-center text-sm text-gray-400 dark:text-slate-500 font-medium">Loading tasks...</div>
          ...
          <div className="p-12 text-center">
            <div className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 flex items-center justify-center mx-auto mb-3">
              <ListTodo className="w-5 h-5 text-gray-400 dark:text-slate-500" />
            </div>
            <p className="text-sm font-semibold text-gray-600 dark:text-slate-300">No tasks found</p>
          ...
          <table className="w-full">
            <thead className="sticky top-0 bg-[#FAFAFA] dark:bg-slate-900 text-gray-900 dark:text-gray-100 z-10">
              <tr className="border-b border-gray-200 dark:border-slate-800">
                <th className="w-10 px-3 py-2.5" />
                <th className="text-left px-3 py-2.5 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">Title</th>
                <th className="text-left px-3 py-2.5 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider w-[120px]">Status</th>
                <th className="text-left px-3 py-2.5 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider w-[100px]">Priority</th>
                <th className="text-left px-3 py-2.5 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider w-[140px]">Assignee</th>
                <th className="text-left px-3 py-2.5 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider w-[130px]">Workspace</th>
                <th className="text-left px-3 py-2.5 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider w-[100px]">Due Date</th>
                <th className="text-left px-3 py-2.5 text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider w-[80px]">Created</th>
                <th className="w-10 px-3 py-2.5" />
              </tr>
            </thead>
            <tbody>
              {filteredTasks.map((task) => {
                ...
                return (
                  <tr key={task._id}
                    ...
                    className="border-b border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-gray-50 dark:bg-slate-800/80 dark:hover:bg-slate-800 transition-colors cursor-pointer group"
                  >
                    {/* Checkbox */}
                    ...
                    <td className="px-3 py-2.5">
                      <span className={`text-[13px] font-semibold leading-tight ${isDone ? "line-through text-gray-400 dark:text-slate-500" : "text-gray-900 dark:text-gray-100"}`}>
                        {task.title}
                      </span>
                      {task.description && (
                        <p className="text-[11px] text-gray-400 dark:text-slate-500 mt-0.5 line-clamp-1">{task.description}</p>
                      )}
                    </td>
                        ...
                        {STATUS_OPTIONS.map(s => {
                          ...
                          return (
                            <button key={s.value}
                              ...
                              className={`w-full text-left px-3 py-1.5 text-[12px] font-medium hover:bg-gray-50 dark:bg-slate-900 flex items-center gap-2 ${
                                task.status === s.value ? "text-[#6366F1]" : "text-gray-700 dark:text-gray-300"
                              }`}
                            >
                      ...
                      >
                        {PRIORITY_OPTIONS.map(p => (
                          <button key={p.value}
                            ...
                            className={`w-full text-left px-3 py-1.5 text-[12px] font-medium hover:bg-gray-50 dark:bg-slate-900 flex items-center gap-2 ${
                              priorityVal === p.value ? "text-[#6366F1]" : "text-gray-700 dark:text-gray-300"
                            }`}
                          >
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <div className="w-5 h-5 rounded-full overflow-hidden bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-800 shrink-0">
                          <img src={assigneeAvatar} className="w-full h-full object-cover" alt="" />
                        </div>
                        <span className={`text-[12px] font-medium truncate ${assignee.name ? "text-gray-700 dark:text-gray-300" : "text-gray-400 dark:text-slate-500"}`}>
                          {assigneeName}
                        </span>
                      </div>
                    </td>
                    ...
                    {/* Workspace */}
                    <td className="px-3 py-2.5">
                      <span className="text-[11px] font-semibold text-[#6366F1] bg-indigo-50 dark:bg-indigo-950/60 px-1.5 py-0.5 rounded">
                        {task.workspaceName}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Dropdown
                        align="right"
                        trigger={
                          <button className="w-6 h-6 flex items-center justify-center rounded hover:bg-gray-100 dark:bg-slate-800 opacity-0 group-hover:opacity-100 transition-opacity text-gray-400 dark:text-slate-500">
                      ...
                      >
                        <button
                          onClick={() => setSelectedTask(task)}
                          className="w-full text-left px-3 py-1.5 text-[12px] font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-slate-900 flex items-center gap-2"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-lg p-6">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setIsNewTaskModalOpen(false)} className="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:text-slate-300">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleCreateTask}>
              <div className="space-y-4">
                {/* Title */}
                <div>
                  <label className="text-[12px] font-semibold text-gray-700 dark:text-gray-300 block mb-1">Title</label>
                  <Input
                    value={newTask.title}
                ...
                {/* Description */}
                <div>
                  <label className="text-[12px] font-semibold text-gray-700 dark:text-gray-300 block mb-1">Description <span className="text-gray-400 dark:text-slate-500 font-normal">(optional)</span></label>
                  <textarea
                    value={newTask.description}
                    onChange={(e) => setNewTask(p => ({ ...p, description: e.target.value }))}
                    placeholder="Add more detail..."
                    rows={2}
                    className="w-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg px-3 py-2 text-sm text-gray-700 dark:text-gray-300 placeholder:text-gray-400 dark:text-slate-500 focus:outline-none focus:ring-2 focus:ring-[#6366F1] resize-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[12px] font-semibold text-gray-700 dark:text-gray-300 block mb-1">Priority</label>
                        ...
                        <button key={p.value} type="button"
                          ...
                          className={`flex-1 py-1.5 rounded-md text-[11px] font-semibold border transition-colors ${
                            newTask.priority === p.value
                              ? p.bg
                              : "bg-gray-50 dark:bg-slate-900 border-gray-200 dark:border-slate-800 text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:bg-slate-800"
                          }`}
                        >
                          {p.label}
                        </button>
                  ...
                  <div>
                    <label className="text-[12px] font-semibold text-gray-700 dark:text-gray-300 block mb-1">Due Date</label>
                    <input
                      type="date"
                      value={newTask.dueDate}
                      onChange={(e) => setNewTask(p => ({ ...p, dueDate: e.target.value }))}
                      className="w-full h-9 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg px-3 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-[#6366F1]"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[12px] font-semibold text-gray-700 dark:text-gray-300 block mb-1">Assignee</label>
                    <select
                  ...
                  {selectedWorkspaceId === "ALL" && (
                    <div>
                      <label className="text-[12px] font-semibold text-gray-700 dark:text-gray-300 block mb-1">Workspace</label>
                      <select
      ...
      {selectedTask && (
        <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 h-full w-full max-w-lg p-6 overflow-y-auto flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
            <div className="flex justify-between items-start pb-4 border-b border-gray-100 dark:border-slate-800 shrink-0">
              <div className="pr-4">
                <h2 className="text-base font-bold text-gray-900 dark:text-gray-100 leading-tight">{selectedTask.title}</h2>
                {selectedTask.description && (
                  <p className="text-[13px] text-gray-500 dark:text-slate-400 mt-1">{selectedTask.description}</p>
                )}
              </div>
              <button onClick={() => setSelectedTask(null)} className="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:text-slate-300 shrink-0 mt-0.5">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 py-5 space-y-5 flex flex-col min-h-0">
              {/* Attributes */}
              <div className="grid grid-cols-2 gap-3 bg-gray-50 dark:bg-slate-900 p-4 rounded-xl border border-gray-100 dark:border-slate-800 shrink-0">
                <div>
                  <span className="text-[11px] font-bold text-gray-400 dark:text-slate-500 block uppercase">Status</span>
                  <Dropdown
                    trigger={
                      <button className={`inline-flex items-center gap-1 mt-1 text-[12px] font-semibold px-2 py-0.5 rounded border ${
                        STATUS_OPTIONS.find(s => s.value === selectedTask.status)?.bg || "bg-gray-50 dark:bg-slate-900 border-gray-200 dark:border-slate-800 text-gray-600 dark:text-slate-300"
                  ...
                  >
                    {STATUS_OPTIONS.map(s => (
                      <button key={s.value}
                        ...
                        className={`w-full text-left px-3 py-1.5 text-[12px] font-medium hover:bg-gray-50 dark:bg-slate-900 flex items-center gap-2 ${
                          selectedTask.status === s.value ? "text-[#6366F1]" : "text-gray-700 dark:text-gray-300"
                        }`}
                      >
                        {s.label}
                      </button>
                ...
                <div>
                  <span className="text-[11px] font-bold text-gray-400 dark:text-slate-500 block uppercase">Priority</span>
                  <Dropdown
                    trigger={
                      <button className={`inline-flex items-center gap-1.5 mt-1 text-[12px] font-semibold px-2 py-0.5 rounded border ${
                        PRIORITY_OPTIONS.find(p => p.value === (selectedTask.priority || "MEDIUM").toUpperCase())?.bg || "bg-gray-50 dark:bg-slate-900 border-gray-200 dark:border-slate-800 text-gray-600 dark:text-slate-300"
                      } hover:opacity-80 transition-opacity`}>
                  ...
                  >
                    {PRIORITY_OPTIONS.map(p => (
                      <button key={p.value}
                        ...
                        className={`w-full text-left px-3 py-1.5 text-[12px] font-medium hover:bg-gray-50 dark:bg-slate-900 flex items-center gap-2 ${
                          (selectedTask.priority || "MEDIUM").toUpperCase() === p.value ? "text-[#6366F1]" : "text-gray-700 dark:text-gray-300"
                        }`}
                      >
                        {p.label}
                      </button>
                ...
                <div>
                  <span className="text-[11px] font-bold text-gray-400 dark:text-slate-500 block uppercase">Workspace</span>
                  <span className="text-[12px] font-semibold text-gray-700 dark:text-gray-300 mt-1 block">{selectedTask.workspaceName}</span>
                </div>
                <div>
                  <span className="text-[11px] font-bold text-gray-400 dark:text-slate-500 block uppercase">Assignee</span>
                  <Dropdown
                    trigger={
                      <button className="flex items-center gap-1.5 mt-1 hover:opacity-80 transition-opacity text-left">
                        ...
                        <span className="text-[12px] font-semibold text-gray-700 dark:text-gray-300">{selectedTask.assignee?.name || "Unassigned"}</span>
                      </button>
                    }
                  >
                    <button
                      ...
                      className="w-full text-left px-3 py-1.5 text-[12px] font-medium hover:bg-gray-50 dark:bg-slate-900 text-gray-400 dark:text-slate-500"
                    >
                      Unassigned
                    </button>
                    {taskWorkspaceMembers.map((m: any) => {
                      ...
                      return (
                        <button key={user._id || m.user}
                          ...
                          className={`w-full text-left px-3 py-1.5 text-[12px] font-medium hover:bg-gray-50 dark:bg-slate-900 ${
                            (selectedTask.assignee?._id || selectedTask.assignee) === (user._id || user) ? "text-[#6366F1]" : "text-gray-700 dark:text-gray-300"
                          }`}
                        >
                          {userName}
                        </button>
                ...
                {selectedTask.dueDate && (
                  <div className="col-span-2 pt-2 border-t border-gray-200 dark:border-slate-800/60 dark:border-slate-800/60">
                    <span className="text-[11px] font-bold text-gray-400 dark:text-slate-500 block uppercase">Due Date</span>
              ...
              <div className="space-y-3 flex-1 flex flex-col min-h-0">
                <h3 className="text-[13px] font-bold text-gray-900 dark:text-gray-100 shrink-0">Comments</h3>
                ...
                <form onSubmit={(e) => { e.preventDefault(); if (!newCommentText.trim()) return; addCommentMutation.mutate(newCommentText) }} className="flex gap-2 shrink-0">
                  <Input
                    ...
                    className="flex-1 bg-gray-50 dark:bg-slate-900 text-sm"
                  />
                    ...
                    comments.map((c: any) => {
                      ...
                      return (
                        <div key={c._id} className="flex gap-2.5 items-start p-3 bg-gray-50 dark:bg-slate-900/50 border border-gray-100 dark:border-slate-800 rounded-lg">
                          <div className="w-6 h-6 rounded-full overflow-hidden shrink-0 border border-gray-200 dark:border-slate-800 bg-gray-100 dark:bg-slate-800">
                            <img src={avatar} className="w-full h-full object-cover" alt="" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-center">
                              <span className="text-[12px] font-bold text-gray-900 dark:text-gray-100">{commenterName}</span>
                              <span className="text-[10px] text-gray-400 dark:text-slate-500">{relativeTime(c.createdAt)}</span>
                            </div>
                            <p className="text-[13px] text-gray-700 dark:text-gray-300 mt-0.5 leading-relaxed break-words">{c.content}</p>
                          </div>
                        </div>
                      )
                    })
                  ) : (
                    <div className="text-center py-6 text-[12px] text-gray-400 dark:text-slate-500">No comments yet.</div>
                  )}
                </div>
              </div>
            </div>
...
export default function TaskBoard() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-gray-500 dark:text-slate-400">Loading Tasks...</div>}>
      <TaskListContent />
    </Suspense>
  )
}
