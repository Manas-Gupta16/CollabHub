...
export default function WorkspaceOverview() {
    ...
    return (
      <div className="flex-1 flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-gray-500 dark:text-slate-400">Loading workspace dashboard...</p>
        </div>
      </div>
    )
  }
  ...
  if (!workspace) {
    return (
      <div className="p-12 text-center">
        <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Workspace not found</h2>
        <p className="text-sm text-gray-500 dark:text-slate-400 mt-1 mb-4">The workspace you are looking for does not exist or you don't have access.</p>
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#FAFAFA] dark:bg-slate-950 text-gray-900 dark:text-gray-100 p-6 lg:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        
        {/* Breadcrumb */}
        <div className="flex items-center text-sm text-gray-500 dark:text-slate-400 gap-2 font-medium">
          <Link href="/workspaces" className="hover:text-gray-900 dark:text-gray-100 transition-colors flex items-center gap-1">
            <Layers className="w-3.5 h-3.5" />
            Workspaces
          </Link>
          <ChevronRight className="w-3.5 h-3.5 text-gray-400 dark:text-slate-500" />
          <span className="text-gray-900 dark:text-gray-100 font-semibold">{workspace.name}</span>
        </div>

        {/* Workspace Header Card (Theme Matched) */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 text-[#6366F1] border border-indigo-100/80 flex items-center justify-center font-bold text-xl shadow-xs shrink-0">
              {initials}
            </div>
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">{workspace.name}</h1>
              ...
              <p className="text-gray-500 dark:text-slate-400 text-xs mt-1 font-medium max-w-xl">
          ...
          <div className="flex items-center gap-2.5 w-full md:w-auto shrink-0">
            <Button 
              variant="outline" 
              className="border-gray-200 dark:border-slate-800 hover:bg-gray-50 dark:bg-slate-800/80 dark:hover:bg-slate-800/80 rounded-xl font-semibold text-xs text-gray-700 dark:text-gray-300 shadow-2xs cursor-pointer" 
              onClick={() => setIsInviteModalOpen(true)}
            >
              <UserPlus className="w-4 h-4 mr-1.5 text-gray-500 dark:text-slate-400" /> Invite Team
            </Button>
            {isOwner && (
              <Button 
                variant="outline" 
                className="border-gray-200 dark:border-slate-800 hover:bg-gray-50 dark:bg-slate-800/80 dark:hover:bg-slate-800/80 rounded-xl p-2.5 text-gray-500 dark:text-slate-400 cursor-pointer"
                onClick={() => router.push(`/settings?workspace=${id}&tab=workspace`)}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-gray-400 dark:text-slate-500 group-hover:text-indigo-600 uppercase tracking-wider transition-colors">Team Members</p>
                <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">{stats?.totalMembers || members.length}</h3>
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Active collaborators</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-[#6366F1] group-hover:bg-indigo-600 group-hover:text-white transition-all flex items-center justify-center shrink-0 shadow-inner">
          ...
          <Card 
            ...
            className="border-gray-200 dark:border-slate-800/80 shadow-sm hover:shadow-md hover:border-purple-300 transition-all rounded-2xl bg-white dark:bg-slate-900 cursor-pointer group active:scale-[0.99]"
            title="Click to open channel messages"
          >
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-gray-400 dark:text-slate-500 group-hover:text-purple-600 uppercase tracking-wider transition-colors">Channels</p>
                <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">{stats?.totalChannels || workspace.channels?.length || 0}</h3>
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Sidebar chat rooms</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-purple-50 dark:bg-purple-950/60 text-purple-600 group-hover:bg-purple-600 group-hover:text-white transition-all flex items-center justify-center shrink-0 shadow-inner">
          ...
          >
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-gray-400 dark:text-slate-500 group-hover:text-amber-600 uppercase tracking-wider transition-colors">Total Tasks</p>
                <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">{totalTasksCount}</h3>
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{completedTasksCount} completed</p>
              </div>
              <div className="w-12 h-12 rounded-xl bg-amber-50 dark:bg-amber-950/60 text-amber-600 group-hover:bg-amber-600 group-hover:text-white transition-all flex items-center justify-center shrink-0 shadow-inner">
            ...
            <CardContent className="p-5 flex items-center justify-between">
              <div className="w-full pr-2">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-xs font-semibold text-gray-400 dark:text-slate-500 group-hover:text-emerald-600 uppercase tracking-wider transition-colors">Progress</p>
              ...
              <div className="w-12 h-12 rounded-xl bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-all flex items-center justify-center shrink-0 shadow-inner ml-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card id="tasks-section" className="border-gray-200 dark:border-slate-800/80 shadow-sm rounded-2xl bg-white dark:bg-slate-900 overflow-hidden scroll-mt-6">
              <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-gray-100 dark:border-slate-800">
                <div>
                  <CardTitle className="text-lg font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                    Tasks & Milestones
                    <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300">
                      {filteredTasks.length}
                    </span>
                  </CardTitle>
                  <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Track and manage workspace deliverables</p>
                </div>
                
                <div className="flex items-center gap-2">
                  <div className="flex items-center bg-gray-100 dark:bg-slate-800 p-1 rounded-xl text-xs font-medium">
                ...
                <div className="divide-y divide-gray-100 dark:divide-slate-800">
                  ...
                  {filteredTasks.length === 0 && (
                    <div className="p-8 text-center">
                      <div className="w-12 h-12 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-gray-400 dark:text-slate-500 mx-auto mb-3">
                        <ListTodo className="w-6 h-6" />
                      </div>
                      <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">No tasks found</p>
                      <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">Create a new task to get started on workspace goals.</p>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card className="border-gray-200 dark:border-slate-800/80 shadow-sm rounded-2xl bg-white dark:bg-slate-900">
                ...
                <div className="space-y-6">
                  {activityList && activityList.length > 0 ? (
                    activityList.slice(0, 8).map((act: any, i: number) => (
                      <div key={i} className="flex gap-4 relative before:absolute before:left-4 before:top-8 before:bottom-[-24px] before:w-px before:bg-gray-200 dark:bg-slate-700/80 last:before:hidden">
                        <div className="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-100 flex items-center justify-center text-[#6366F1] z-10 shrink-0 shadow-xs">
                          <Activity className="w-4 h-4" />
                        </div>
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                            <span className="font-bold text-gray-900 dark:text-gray-100">{act.user?.name || 'A team member'}</span> {act.details || (act.action ? act.action.replace(/_/g, ' ').toLowerCase() : '')}
                          </p>
                    ...
                    <div className="flex gap-4 relative">
                      <div className="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-100 flex items-center justify-center text-[#6366F1] z-10 shrink-0">
                        <Activity className="w-4 h-4" />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">Workspace initialized</p>
                        <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">Ready for team activity updates.</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-6">
            ...
            {/* Team Members Card */}
            <Card id="team-members-section" className="border-gray-200 dark:border-slate-800/80 shadow-sm rounded-2xl bg-white dark:bg-slate-900 overflow-visible scroll-mt-6">
              <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-gray-100 dark:border-slate-800">
                <CardTitle className="text-lg font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                  Team Members
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 border border-indigo-100">
                    {members.length}
                  </span>
                </CardTitle>
                <Button 
                  ...
                  size="icon"
                  onClick={() => setIsInviteModalOpen(true)}
                  className="h-8 w-8 text-indigo-600 hover:bg-indigo-50 dark:bg-indigo-950/60 rounded-lg cursor-pointer"
                  title="Invite Member"
                >
              ...
              <CardContent className="pt-4 overflow-visible">
                <div className="space-y-3.5">
                  {(showAllMembers ? members : members.slice(0, 8)).map((member: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between p-2 rounded-xl hover:bg-gray-50 dark:bg-slate-800/80 dark:hover:bg-slate-800/80 transition-colors">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-gray-400 dark:text-slate-500 truncate flex items-center gap-1">
                              <span>{user.email || 'Member'}</span>
                              <span>•</span>
                              <span className={isOnline ? 'text-emerald-600 font-semibold' : 'text-gray-400 dark:text-slate-500'}>{isOnline ? 'Online' : 'Offline'}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded-md uppercase tracking-wider ${
                            role === 'OWNER' ? 'bg-amber-50 dark:bg-amber-950/60 text-amber-700 border border-amber-200/60' :
                            role === 'ADMIN' ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 border border-indigo-200/60' :
                            'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-800/60'
                          }`}>
                            {role}
                          </span>
                          ...
                          {isOwner && role !== 'OWNER' && (
                            <Dropdown
                              align="right"
                              trigger={
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-gray-400 dark:text-slate-500 hover:text-gray-700 dark:text-gray-300 rounded-lg cursor-pointer">
                            ...
                            >
                              {role === 'MEMBER' ? (
                                <button
                                  ...
                                  className="w-full text-left px-3 py-2 text-[12px] font-semibold text-indigo-600 hover:bg-indigo-50 dark:bg-indigo-950/60 flex items-center gap-2 cursor-pointer"
                                >
                                ...
                                <button
                                  ...
                                  className="w-full text-left px-3 py-2 text-[12px] font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:bg-slate-800/80 dark:hover:bg-slate-800 flex items-center gap-2 cursor-pointer"
                                >
                                  <Shield className="w-3.5 h-3.5 text-gray-400 dark:text-slate-500" /> Make Member
                                </button>
                              )}
                ...
                {members.length > 8 && !showAllMembers && (
                  <Button 
                    variant="outline" 
                    className="w-full mt-4 border-gray-200 dark:border-slate-800 text-gray-700 dark:text-gray-300 rounded-xl text-xs font-semibold hover:bg-gray-50 dark:bg-slate-800/80 dark:hover:bg-slate-800 cursor-pointer" 
              ...
              <div className="relative z-10">
                ...
                <div className="mt-4 flex gap-2">
                  <Button 
                    ...
                    className="bg-white text-indigo-900 hover:bg-indigo-50 dark:bg-indigo-950/60 font-semibold rounded-xl text-xs shadow-sm cursor-pointer"
                  >
                    Open Settings
                  </Button>
                </div>
        ...
        {isInviteModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4 animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-100 dark:border-slate-800">
              ...
              <p className="text-xs text-gray-500 dark:text-slate-400 mb-4">Send an invitation to join <strong className="text-gray-800 dark:text-gray-200">{workspace.name}</strong>.</p>
              
              <form onSubmit={handleInvite}>
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-bold uppercase text-gray-500 dark:text-slate-400 tracking-wider block mb-1.5">Email Address</label>
                    <Input 
                      type="email"
                      value={inviteEmail} 
                      onChange={(e) => setInviteEmail(e.target.value)} 
                      placeholder="e.g. teammate@collabhub.com" 
                      className="rounded-xl border-gray-200 dark:border-slate-800 focus:ring-2 focus:ring-indigo-500"
                      required
                    />
                  </div>
                </div>
                <div className="flex justify-end gap-3 mt-6">
                  <Button type="button" variant="outline" className="rounded-xl border-gray-200 dark:border-slate-800 cursor-pointer" onClick={() => setIsInviteModalOpen(false)}>Cancel</Button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4 animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-100 dark:border-slate-800">
              ...
              <form onSubmit={handleCreateTask}>
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-bold uppercase text-gray-500 dark:text-slate-400 tracking-wider block mb-1.5">Task Title</label>
                    <Input 
                      value={newTaskTitle} 
                  ...
                  <div>
                    <label className="text-xs font-bold uppercase text-gray-500 dark:text-slate-400 tracking-wider block mb-1.5">Assignee</label>
                    <select
                ...
                <div className="flex justify-end gap-3 mt-6">
                  <Button type="button" variant="outline" className="rounded-xl border-gray-200 dark:border-slate-800 cursor-pointer" onClick={() => setIsNewTaskModalOpen(false)}>Cancel</Button>
