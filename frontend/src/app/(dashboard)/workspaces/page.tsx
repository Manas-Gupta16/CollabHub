  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#FAFAFA] dark:bg-slate-950 text-gray-900 dark:text-gray-100 p-6 lg:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-200 dark:border-slate-800/80 shadow-xs">
          <div>
            <div className="flex items-center gap-2.5">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Workspaces</h1>
              {workspaces && (
            ...
            <p className="text-gray-500 dark:text-slate-400 text-sm mt-1">Manage and collaborate across team projects, channels, and deliverables.</p>
          </div>
          <Button 
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800/80 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all shadow-2xs placeholder:text-gray-400 dark:text-slate-500 font-medium"
            />
          </div>
          <div className="flex gap-2 shrink-0">
            <select
              ...
              className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800/80 rounded-xl px-3 py-2.5 text-xs font-semibold text-gray-700 dark:text-gray-300 shadow-2xs focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
            >
        ...
        {isLoading ? (
          <div className="flex items-center justify-center min-h-[300px]">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm font-medium text-gray-500 dark:text-slate-400">Loading workspaces...</p>
            </div>
          </div>
        ) : filteredWorkspaces.length === 0 ? (
          <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800/80 shadow-xs">
            <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-950/60 text-[#6366F1] flex items-center justify-center mx-auto mb-3">
              ...
              return (
                ...
                >
                  <Link href={`/workspaces/${ws._id}`}>
                    <Card className="hover:shadow-lg hover:border-indigo-300 transition-all cursor-pointer border-gray-200 dark:border-slate-800/80 overflow-hidden group bg-white dark:bg-slate-900 rounded-2xl flex flex-col h-full">
                        ...
                        <div>
                          ...
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <span className={`text-[10px] font-bold px-2.5 py-1 rounded-lg border flex items-center gap-1 uppercase tracking-wider ${
                              userRole === 'Owner' ? 'bg-amber-50 dark:bg-amber-950/60 text-amber-700 border-amber-200/60' :
                              userRole === 'Admin' ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 border-indigo-200/60' :
                              'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-300 border-gray-200 dark:border-slate-800/60 dark:border-slate-800/60'
                            }`}>
                          ...
                          {/* Title & description */}
                          <h3 className="text-base font-bold text-gray-900 dark:text-gray-100 group-hover:text-[#6366F1] transition-colors line-clamp-1 mb-1">
                            {ws.name}
                          </h3>
                          <p className="text-xs text-gray-500 dark:text-slate-400 line-clamp-2 leading-relaxed h-8">
                            {ws.description || 'Team workspace for projects and chat channels.'}
                          </p>
                          
                          {/* Meta stats */}
                          <div className="flex items-center gap-4 mt-4 text-xs font-medium text-gray-500 dark:text-slate-400 border-t border-gray-100 dark:border-slate-800 pt-3">
                        ...
                        <div className="mt-5 pt-3 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
                          <div className="flex -space-x-2">
                            ...
                            {(ws.members?.length || 0) > 4 && (
                              <div className="w-7 h-7 rounded-full border-2 border-white bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-[9px] font-bold text-gray-600 dark:text-slate-300">
                                +{ws.members.length - 4}
                              </div>
                            )}
                          </div>
          ...
          {isModalOpen && (
            ...
            >
              <motion.div 
                ...
                className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-100 dark:border-slate-800"
              >
                ...
                <p className="text-xs text-gray-500 dark:text-slate-400 mb-4">Set up a new workspace for your team and projects.</p>

                <form onSubmit={handleCreate}>
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-bold uppercase text-gray-500 dark:text-slate-400 tracking-wider block mb-1.5">Workspace Name</label>
                      <Input 
                    ...
                    <div>
                      <label className="text-xs font-bold uppercase text-gray-500 dark:text-slate-400 tracking-wider block mb-1.5">Description</label>
                      <Input 
                  ...
                  <div className="flex justify-end gap-3 mt-6">
                    <Button type="button" variant="outline" className="rounded-xl border-gray-200 dark:border-slate-800 cursor-pointer" onClick={() => setIsModalOpen(false)}>Cancel</Button>
