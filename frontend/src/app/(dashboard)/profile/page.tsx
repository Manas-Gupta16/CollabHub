"use client"

import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Mail, MapPin, Link as LinkIcon, Calendar, CheckCircle2, MessageSquare, GitCommit } from "lucide-react"
import { useQuery } from "@tanstack/react-query"
import api, { getFileUrl } from "@/lib/api"
import { UserAvatar } from "@/components/UserAvatar"

export default function ProfilePage() {
  const { data: user } = useQuery({
    queryKey: ['profile'],
    queryFn: async () => {
      const res = await api.get('/auth/profile')
      return res.data?.user
    },
  })

  const userName = user?.name || 'User'
  const userEmail = user?.email || 'user@collabhub.com'
  const userSeed = userName.replace(/\s/g, '')

  return (
    <div className="flex-1 overflow-y-auto bg-gradient-to-br from-[#F5F8FF] to-[#E9F0FE] dark:from-slate-950 dark:to-slate-900 p-8 relative">

      <div className="max-w-4xl mx-auto space-y-6 relative z-10">
        
        {/* Profile Header Card */}
        <Card className="overflow-hidden border-gray-200 dark:border-slate-800/60 dark:border-slate-800/60 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
          <div className="h-32 bg-gradient-to-r from-[#6366F1] to-purple-500 w-full relative">
            <Button variant="outline" className="absolute top-4 right-4 bg-white/20 dark:bg-slate-800/20 text-white border-white/40 dark:border-slate-700/40 hover:bg-white/30 dark:hover:bg-slate-800/30 hover:text-white">
              Edit Cover
            </Button>
          </div>
          <CardContent className="p-8 pt-0 relative">
             <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-end -mt-12 mb-6">
               <UserAvatar name={userName} avatar={user?.avatar} size="w-24 h-24 text-3xl font-bold border-4 border-white dark:border-slate-900 shadow-md" />
               <div className="flex-1">
                 <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">{userName}</h1>
                 <p className="text-gray-500 dark:text-slate-400 font-medium">{user?.title || 'Team Member at CollabHub'}</p>
               </div>
               <div className="flex gap-2 w-full sm:w-auto">
                 <Button variant="outline" className="flex-1 sm:flex-none border-gray-200 dark:border-slate-800">Share</Button>
                 <a href="/settings"><Button className="flex-1 sm:flex-none bg-[#6366F1] hover:bg-[#4F46E5] text-white">Edit Profile</Button></a>
               </div>
             </div>

             <div className="flex flex-wrap gap-y-2 gap-x-6 text-sm text-gray-600 dark:text-slate-300">
               {user?.location && (
                 <div className="flex items-center gap-1.5"><MapPin className="w-4 h-4 text-gray-400 dark:text-slate-500"/> {user.location}</div>
               )}
               <div className="flex items-center gap-1.5"><Mail className="w-4 h-4 text-gray-400 dark:text-slate-500"/> {userEmail}</div>
               {user?.website && (
                 <div className="flex items-center gap-1.5"><LinkIcon className="w-4 h-4 text-gray-400 dark:text-slate-500"/> {user.website}</div>
               )}
               <div className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-gray-400 dark:text-slate-500"/> Joined {user?.createdAt ? new Date(user.createdAt).getFullYear() : 2026}</div>
             </div>
          </CardContent>
        </Card>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          
          {/* Left Column */}
          <div className="lg:col-span-1 space-y-6">
            <Card className="border-gray-200 dark:border-slate-800/60 dark:border-slate-800/60 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
              <CardContent className="p-6">
                 <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-4">About</h2>
                 <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed mb-6">
                   {user?.bio || 'Passionate about creating intuitive, user-centric experiences. Building the future of collaboration at CollabHub.'}
                 </p>
                 <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-4">Stats</h2>
                 <div className="space-y-4">
                   <div className="flex justify-between items-center">
                     <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-300"><CheckCircle2 className="w-4 h-4 text-emerald-500"/> Tasks Done</div>
                     <span className="font-bold text-gray-900 dark:text-gray-100">142</span>
                   </div>
                   <div className="flex justify-between items-center">
                     <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-300"><MessageSquare className="w-4 h-4 text-blue-500"/> Messages</div>
                     <span className="font-bold text-gray-900 dark:text-gray-100">2.4k</span>
                   </div>
                   <div className="flex justify-between items-center">
                     <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-slate-300"><GitCommit className="w-4 h-4 text-purple-500"/> Commits</div>
                     <span className="font-bold text-gray-900 dark:text-gray-100">45</span>
                   </div>
                 </div>
              </CardContent>
            </Card>
          </div>

          {/* Right Column */}
          <div className="lg:col-span-2 space-y-6">
            <Card className="border-gray-200 dark:border-slate-800/60 dark:border-slate-800/60 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
              <CardContent className="p-6">
                <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-6">Recent Activity</h2>
                <div className="space-y-6">
                  {[
                    { title: 'Uploaded a new asset', target: 'Landing_Page_v2.pdf', time: '2 hours ago', icon: <CheckCircle2 className="text-emerald-500 w-4 h-4"/>, bg: 'bg-emerald-50 dark:bg-emerald-950/60' },
                    { title: 'Commented on', target: 'Design System Typography', time: '4 hours ago', icon: <MessageSquare className="text-blue-500 w-4 h-4"/>, bg: 'bg-blue-50 dark:bg-blue-950/60' },
                    { title: 'Completed task', target: 'Update Dashboard Layout', time: 'Yesterday', icon: <CheckCircle2 className="text-emerald-500 w-4 h-4"/>, bg: 'bg-emerald-50 dark:bg-emerald-950/60' }
                  ].map((activity, i) => (
                    <div key={i} className="flex gap-4 relative before:absolute before:left-4 before:top-8 before:bottom-[-24px] before:w-px before:bg-gray-200 dark:bg-slate-700 last:before:hidden">
                      <div className={`w-8 h-8 rounded-full ${activity.bg} flex items-center justify-center z-10 shrink-0`}>
                         {activity.icon}
                      </div>
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{activity.title} <span className="font-bold">{activity.target}</span></p>
                        <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">{activity.time}</p>
                      </div>
                    </div>
                  ))}
                </div>
              </CardContent>
            </Card>
          </div>

        </div>

      </div>
    </div>
  )
}
